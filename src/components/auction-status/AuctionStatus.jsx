...
const AuctionStatus = ({ highestBid, highestBidder, endDate }) => {
	...
	if (highestBid === 0)
		return <p>No hay ningún pujador, ¡puedes ser el primero!</p>;

	if (timeSpan <= 0 && highestBidder === loggedUser.email)
		return <p>¡Enhorabuena! Has ganado la subasta</p>;

	if (highestBidder === loggedUser.email) return <p>Eres el mayor postor</p>;

	if (timeSpan <= 0) return <p>La subasta ha finalizado</p>;
